function [solution,score,reactionsToKeep, LogData] = runKApproxmitationIrrevTROR_AdjustmentV4Testing (taskModel, expValueMeanAdjustedTask, epsilon, rev2irrev, alpha, beta, i,j,osenseStr,irrev,orphanHandling, changedValue)
...
% Parameters tested
% varargin.Heuristics = changedValue;
...
noWeirdFeasibleSolutionsInInfeasibleSpace = false;
...
kInfeasibletest =  ((2*alpha)* (max(expValueMeanAdjustedTask) +TransportReactionPenalty)) + (2*(1-alpha)*beta) + TransportReactionPenalty;%
counter = 0;
...
fprintf("timeLimit_ConstantFirstFeas = %i, timeLimit_ConstantHeursticMultipleFeasilibity = %î\n difference_ConstantfeasibleHeuristic = %i, amountOfTryMoreTries = %i\n",timeLimit_ConstantFirstFeas,timeLimit_ConstantHeursticMultipleFeasilibity,difference_ConstantfeasibleHeuristic ,amountOfTryMoreTries)
fprintf("AdjustBounds = %s, AdjustBoundsValue = %î\n",mat2str(AdjustBounds),AdjustBoundsValue)
...
%MILPproblem = load('LastCreatedMILPproblem_Task26Sample1_date_2023-10-25___14-47-20.mat').MILPproblem;

currentDateTime = datestr(now, 'yyyy-mm-dd___HH-MM-SS');
filenameMILP = sprintf('LastCreatedMILPproblem_Task%iSample%i_date_%s.mat', i,j,currentDateTime);
...
%fprintf("kFeasible = %.3f ___ kInfeasible = %.3f ___ TransportReactionPenalty = %i \n",kfeasibletest,kInfeasibletest,TransportReactionPenalty)
...
while ~ExceededInitialInfeasConstraint && kdiff >= kTolerance
    ...
    MILPproblem = adaptKMILPProblemAverageExpressionTROR_AdTrans(MILPproblem, taskModel, expValueMeanAdjustedTask, ktest,transportRxnIDs, alpha, beta,x0hint,x0ON,TransportReactionPenalty);
    logFile = convertStringsToChars(sprintf("Infeasible_Space_run_%i.txt",infeasibleCounter));
    ...
    if solution.stat == 1|| solution.stat == 10 % Optimal (1) or feasible (10) found 
        ...
        validValues = expValueMeanAdjustedTaskForComparison(selectedRows) ~= -1;
        selectedExpression = expValueMeanAdjustedTaskForComparison(selectedRows);        
        ...
        getActiveReactionsWithFluxesFromModelAndIrrevTROR_inter_save(taskModel, lastValidSolution,expValueMeanAdjustedTaskForComparison, kfeasibletest , i, osenseStr, irrev,j, alpha, beta, epsilon);
        if solution.stat == 1
...
%UseSpecialYflags = false; % did not seem to make it faster
...
while (~approximatedOptimalSolutionThroughTimeHeuristic && ~couldNotFindAnySolution && kdiff >= kTolerance)
    ...
    timerStart = tic;
    rng(1) % unfortunately only gurobi method 4 and 5 are deterministic
    logFile = convertStringsToChars(sprintf("feasible_Space_run_%i.txt",feasibleCounter));
...
%% Feasibility testing using normal settings (found a solution using
%%  heuristics for task 20 where the above approach didn't so it seems
%%  worth trying
if approximatedOptimalSolutionThroughTimeHeuristic
    ...
    while ~ExceededInitialInfeasConstraint && kdiff >= kTolerance
        ...
        rng(1) % unfortunately only gurobi method 4 and 5 are deterministic
        MILPproblem = adaptKMILPProblemAverageExpressionTROR_AdTrans(MILPproblem, taskModel, expValueMeanAdjustedTask, ktest,transportRxnIDs, alpha, beta,x0hint,x0ON,TransportReactionPenalty);
        logFile = convertStringsToChars(sprintf("feasible_Space_After_feasible_run_%i.txt",infeasibleCounter));
        varargin2.logFile = logFile;
        solution = solveCobraMILP_Adjusted(MILPproblem,varargin2);
        fprintf("Solstat = %i\n", solution.stat)
...
if ExceededInitialInfeasConstraint
    ...
    while ~ExceededInitialInfeasConstraint && kdiff >= kTolerance
        ...
        rng(1) % unfortunately only gurobi method 4 and 5 are deterministic
        MILPproblem = adaptKMILPProblemAverageExpressionTROR_AdTrans(MILPproblem, taskModel, expValueMeanAdjustedTask, ktest,transportRxnIDs, alpha, beta,x0hint,x0ON,TransportReactionPenalty);
        logFile = convertStringsToChars(sprintf("Infeasible_Space_After_feasible_run_%i.txt",infeasibleCounter));
        varargin2.logFile = logFile;
        solution = solveCobraMILP_Adjusted(MILPproblem,varargin2);
        ...
        if solution.stat == 1|| solution.stat == 10
            ...
            getActiveReactionsWithFluxesFromModelAndIrrevTROR_inter_save(taskModel, ...
                ...
                kfeasibletest , i, osenseStr, irrev,j, alpha, beta, epsilon);
            %fprintf(solution.stat)
...
if kdiff > kTolerance
    ...
    fclose(f);
elseif exist('lastValidSolution')
    f = fopen('__converged', 'wt');
    fclose(f);
else
    f = fopen('__convergedButNoSol', 'wt');
    fclose(f);
end
