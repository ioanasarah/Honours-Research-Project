function [taskReport, essentialRxns, taskStructure,usedRxns]=checkMetabolicTasksHumanGEM_2023_2_Jelle(model,param)
% Modified to get usedRxns, fixed some errors in reaction naming, and fixed
% to use essentialRxnsTasks2023_1 and generateTaskStructure_2022
% 
...
% USAGE:
%
%    [taskReport, essentialRxns, taskStructure,usedRxns]=checkMetabolicTasksHumanGEM_2023_1(model,inputFile,printOutput,printOnlyFailed,printDetails,getEssential,taskStructure,saveUsed,relaxation)
%
% INPUTS:
%    model:              a model structure
%    param: a structure containing teh following fields
%                      Mandatory: 
...
%                        parseTaskList for details (ignored if taskStructure is
%                        supplied)
%                           
%                      Optional:
%
...
%                           return usedRxns
%                         -relaxation:           Value by which the initial minimum flux through
%                           the task can be relaxed to define essential reactions. As a general
%                           rule, the higher the relaxation the more strict. An input of Inf will
%                           allow any solution with nonzero total flux. (Default = 1e-06).
%                           Importantly in the base cobraToolbox there is no relaxation at all.
%    
%
% OUTPUTS:
...
%       - Adapted by Jelle Bonthuis, added for possibility to add both
%       forward and backward exchange reacitons
if nargin < 2
    error('At least some parameters have to be set!')
end

if ~isfield(param,'relaxation')
    relaxation = 1e-06;
else 
    relaxation = param.relaxation;
end

if ~isfield(param,"saveUsed")
    saveUsed = false;
else
    saveUsed = param.saveUsed;
end

if ~isfield(param,"getEssential")
     getEssential=false;
else 
    getEssential = param.getEssential;
end

if ~isfield(param,"printOutput")
    printOutput=true;
else
    printOutput = param.printOutput;
end

if ~isfield(param,"printOnlyFailed")
    printOnlyFailed=false;
else
    printOnlyFailed = param.printOnlyFailed;
end

if ~isfield(param,"printDetails")
    printDetails=false;
else
    printDetails = param.printDetails;
end

if ~isfield(param,'taskStructure')
    if ~isfield(param,'inputFile')
        error('You need to provide tasks as a structure of file path!')
    
    else 
        taskStructure = generateTaskStructure_2022(param.inputFile); 
    end

else
    taskStructure = param.taskStructure;
end
            ...
            if ismember(upper(INPUT(end-1)),compSymbol) || ismember(upper(INPUT(end)),compSymbol) 
                ...
                if ~isempty(find(Nb_React==1))
                    ...
                    if ismember(INPUT,taskStructure(i).outputs)==1
                        ...
                        rxnLength = length(tModel.rxns(Tsp_ID(ID_exc))); % EDITIED BY JELLE, NECESSARY FOR REVERISBLE REACTIONS
                        rxn_Subs(end+1:end+rxnLength) = tModel.rxns(Tsp_ID(ID_exc)); 
                    else
            ...
            if ismember(upper(OUTPUT(end-1)),compSymbol)==1 || ismember(upper(OUTPUT(end)),compSymbol)==1
                ...
                if ~isempty(find(Nb_React==1))
                	ID_exc=find(Nb_React==1);
                     ...
                     rxnLength = length(tModel.rxns(Tsp_ID(ID_exc))); % EDITIED BY JELLE, NECESSARY FOR REVERISBLE REACTIONS
                     rxn_Prod(end+1: end+rxnLength)=tModel.rxns(Tsp_ID(ID_exc));
                else
                    AddExchange=1;
                end
            else
        ...
        if sum(abs(sol.full))~=0
            if taskStructure(i).shouldFail==0
                ...
                if getEssential==true 
                    if saveUsed
                        [Rxns_taskEssential,Rxns_Used]=essentialRxnsTasks2023_1(tModel,true,relaxation);
                        usedRxns{i} = Rxns_Used;
                        essentialRxns{i}= Rxns_taskEssential';
                    else
                        [Rxns_taskEssential]=essentialRxnsTasks2023_1(tModel,false,relaxation);
                        essentialRxns{i}= Rxns_taskEssential';
                    end
