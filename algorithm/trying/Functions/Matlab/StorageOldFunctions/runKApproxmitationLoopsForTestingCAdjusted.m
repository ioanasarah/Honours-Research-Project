function [solution,score,reactionsToKeep] = runKApproxmitationLoopsForTestingCAdjusted(taskModel, expValueMeanAdjustedTask, epsilon)

timerval = tic;
kPercentageConstant = 0.7;
ktest = 6.5;    
...
MILPproblem = createKMILPProblemAverageExpressionLoopsForTestingCAdjusted(taskModel, expValueMeanAdjustedTask, epsilon, ktest);
...
while abs(kfeasibletest - kInfeasibletest) > kTolerance
   % ktest = 0;
    ...
    %MILPproblem = createKMILPProblemAverageExpression(taskModel, expValueMeanAdjustedTask, epsilon, rev2irrev, ktest);
    MILPproblem = adaptKMILPProblemAverageExpressionLoops(MILPproblem, taskModel, expValueMeanAdjustedTask, ktest);
    spy(MILPproblem.A)
    % Calculate solution
