function[score, score_binary ,taskInfos, detailScoring]=CellFie_AverageGeneExpression_2023(data,tasksData,model,param)
...
%% Compute the score
%model = closeExchangeReactions(model,taskInfos);
            ...
            if ~isempty(noGene)
           	...
           	if ~isempty(expValue)
            	...
            	if size(expValue,1)>1                    
                    ...
                    param.inputFile = TaskList(inputFile);
                    
                    % Close exchange reactions and set task exchange reactions open
                    taskStructure = tasksData.taskStructure;
                    [PassTask, taskModel] = openExchangeTaskReactionsAndCheckTask(model,taskStructure,i);
                    if passtask
                        MILPproblem = FractionalProgramming_ConstructMILP(taskModel);
                        [reactionsToKeep, score MILPSolution] = runMILPProblemAndGetReactionsToKeep(MILPproblem);
                        taskThreshold   = 5*log(2);
                        if score > taskThreshold
                            %include task
                        end
                    end
