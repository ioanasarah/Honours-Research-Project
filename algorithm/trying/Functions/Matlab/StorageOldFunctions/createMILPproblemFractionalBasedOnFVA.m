function MILPproblem = createMILPproblemFractionalBasedOnFVA(taskModel, expressionRxns, epsilon,TurnOnLoopConstraints,FVAResults)
tic
...
convertedResults = abs(FVAResults) <= 0;
rxnNameList = model.rxns(convertedResults);
taskModel = changeRxnBounds(model, rxnNameList, 0);

S(:,convertedResults) = 0;
...
%lb(convertedResults) = 0;
%ub(convertedResults) = 0;

for i = 1:length(edgeIndices)
    if(convertedResults(i)== 0)
        %Y+ LB -epsilon
        ...
        j(count) = i + 5*size(S,2); %%% CHanged to 5 since
        v(count) = 1;
