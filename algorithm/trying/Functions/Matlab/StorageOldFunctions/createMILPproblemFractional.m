function MILPproblem = createMILPproblemFractional(model, expressionRxns, epsilon,TurnOnLoopConstraints, OrphanReactionValue)
tic
...
% c_z = MILPproblem.c(4*length(model)+1:5*length(model))
% c_z(orphanReactionsIds)
% 4*length(model)+1:5*length(model) 
% 5*length(model)+1:6*length(model) 
