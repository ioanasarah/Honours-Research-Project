...
dispName =  sprintf("MinimumReactionsTask%iSample%iScored%.3f%s%s%s",i,j, score, osenseStr,irrevStr);

removeRows = newVector == 0;

newModel = removeRxns(taskModel, taskModel.rxns(removeRows));

solutionName = string("lastValidSolutionMiniReactions"+dispName+".mat");
newModelName = string("newModelMiniReactions"+dispName+".mat");
fluxSolutionName = string("fluxTableMiniReactions"+dispName+".mat");
