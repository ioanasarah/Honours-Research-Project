function ActiveReactionFluxes = getActiveReactionsWithFluxesFromModelAndIrrevIMATMinimization(taskModel, solution,expValueMeanAdjustedTask,score , i, osenseStr , irrev,j, alpha, beta, epsilon)
...
dispName =  sprintf("Task%iSample%iScored%.3fAverage%.3falpha%.3fbeta%.3f",i,j,score,averageScore, alpha,beta);
...
% newVector(round(abs(solution.cont(1:reactionLength)),6)>= (epsilon-0.01)) = 1;
selectedRows = (1:reactionLength); % = newVector == 1;
...
newCellArray = [num2cell(ones(reactionLength,1)), num2cell(solutionLP.v(selectedRows)), num2cell(expValueMeanAdjustedTask(selectedRows)), taskModel.rxns(selectedRows), taskModel.rxnNames(selectedRows),rxnFormulas];
...
solutionName = string("lastValidSolutionMILP_IMATAdjusted_"+dispName+".mat");
newModelName = string("newModelMILPI_MATAdjusted_"+dispName+".mat");
fluxSolutionName = string("fluxTableMILP_IMATAdjusted_"+dispName+".mat");
