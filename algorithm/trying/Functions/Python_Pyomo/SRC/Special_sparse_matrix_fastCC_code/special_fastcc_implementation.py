""" This is taken from cobrapy and modified to return ids of reactions that were removed, as provided in the Matlab formulation (in order to properly remove bound variables to each reaction). Cobrapy is licensed under GNU GPL v2.0.
## Licensing
...
def fastcc(
    ...
    while rxns_to_check:
        ...
        with model:
            ...
            print("rxns_to_keep", len(rxns_to_keep))
    ...
    indices_to_remove = [model.reactions.index(rxn) for rxn in consistent_rxns]

    consistent_model = model.copy()
    consistent_model.remove_reactions(rxns_to_remove, remove_orphans=True)

    return consistent_model, indices_to_remove
