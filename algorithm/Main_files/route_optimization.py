...
class ModelCreator:
    ...
    def check_if_task_not_mass_charge_balanced(self):
    ...
    def check_if_task_is_viable(self) -> bool:
        ...
        if solution_.status != "optimal":
            return False
        if self.check_if_task_not_mass_charge_balanced():
            return False
        return True
    ...
    def save_results(self, task_number: int, sample_number: int):
        ...
        if self.minimize_filtered_model:
            ...
            # TODO add option to allow for saving both the minimized and non-minimized model (would require making deepcopies of the model and reaction_flux_dict)
            # TODO and then saving both of them with an additional flag to add "minimized" to the filenames
    ...
    def main_optimization_loop(self):
        ...
        metadata_file = f"{self.result_output_folder}\\{self.name_of_run}.json"
        ModelCreator.save_with_tries(
            metadata_file,
            self.settings,
            dump,
            False,
        ...
        while True:
            if sample_counter == len(self.samples_to_run) or next_task:
                ...
                if not self.check_if_task_is_viable():
                    warning_message = f"Task {task_number} is not viable (infeasible cobra.optimize)."
...
if __name__ == "__main__":
    ...
    methods_dict_for_improved = {
        "__global_options": {
            "epsilon": 1e-4,
            "expressionless_reaction_value": 15,
            "create_log_file": True,
            "tasks": [15,16,17],
            "tasks_not_to_run": [116],
            ...
            # "result_output_folder": r"E:\Git\Metabolic_Task_Score\Data\Results\Runs\Reformulated_code\consensus_2024_DCM_magnet_metabolic_tasks_2024_v1_01\\",
            "result_output_folder": r"C:\Users\Jelle\OneDrive\Documenten\GitHub\Metabolic_Task_Score\Data\Results\Runs\Reformulated_code\consensus_2024_DCM_magnet_metabolic_tasks_2024_v1_01",
            "name_of_run": f"Check_why_16_doesnt_run",
            "create_csv_files": True,
            "overwrite_run_files": True, # if True, will not try to create new file names but instead overwrite existing files
    ...
    # main_data_folder = r"E:\Git\Metabolic_Task_Score\Data\Main_files\For_running"
