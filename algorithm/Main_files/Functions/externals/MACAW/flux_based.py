...
from Functions.externals.MACAW.fva import fva
from Functions.externals.MACAW.utils import add_reaction_equations, edit_dead_end_bounds
import Functions.externals.MaCSBio_cobrapy as cobra
from Functions.externals.MACAW.structural import dead_end_test, _dead_end_test_inner
from Functions.externals.MACAW.dilution import constrain_reversible_rxns, add_leakage_reactions
from pebble import ProcessPool, ProcessExpired
from concurrent.futures import TimeoutError
from Functions.externals.MACAW.dilution import add_dilution_constraints
import pandas as pd
...
def dilution_test(
    ...
    while mets_to_test:
        ...
        i = 0
        if attempts == 1 and i == 0: # ADDED BY JELLE BONTHUIS 2025-03-03 TO ALLOW FOR RESTARTING
            # verify that that this works
            if os.path.exists(os.path.join(save_location, f"to_save_dict_temporary.pkl")):
                with open(os.path.join(save_location, f"to_save_dict_temporary.pkl"), "rb") as f:
                    to_save_dict = load(f)
                    mets_to_test = to_save_dict["mets_to_test"]
                    met_dict = to_save_dict["met_dict"]
                    rxn_dict = to_save_dict["rxn_dict"]
                    fva_before = to_save_dict["fva_before"]
                    i = to_save_dict["i"]
                    attempts = to_save_dict["attempts"]
        while True:
            to_save_dict = {
                "mets_to_test": mets_to_test,
                "met_dict": met_dict,
                "rxn_dict": rxn_dict,
                "fva_before": fva_before,
                "i": i,
                "attempts": attempts
            }
            if save_location is not None:
                with open(os.path.join(save_location, f"to_save_dict_temporary.pkl"), "wb") as f:
                    dump(to_save_dict, f)
            try:
    ...
    # ADDED BY JELLE BONTHUIS 2025-03-03 TO ALLOW FOR RESTARTING
    # now hat the entire thing has succesfully ran, we will rename the temporary file to the final file
    if os.path.exists(os.path.join(save_location, f"to_save_dict_temporary.pkl")): # verify that this works
        os.rename(os.path.join(save_location, f"to_save_dict_temporary.pkl"), os.path.join(save_location, f"to_save_dict_final.pkl"))
