...
from Functions.externals.MaCSBio_cobrapy.util import get_context


if TYPE_CHECKING:
    from Functions.externals.MaCSBio_cobrapy import Gene, Metabolite, Model, Reaction
...
def prune_unused_metabolites(model: "Model") -> Tuple["Model", List["Metabolite"]]:
    ...
    list of Functions.externals.MaCSBio_cobrapy.Metabolite
...
def prune_unused_reactions(model: "Model") -> Tuple["Model", List["Reaction"]]:
    ...
    list of Functions.externals.MaCSBio_cobrapy.Reaction
...
def knock_out_model_genes(
    ...
    gene_list: list of Functions.externals.MaCSBio_cobrapy.Gene
        The list of genes to knock-out.

    Returns
    -------
    list[Functions.externals.MaCSBio_cobrapy.Reaction]
        A list of Functions.externals.MaCSBio_cobrapy.Reactions that had the bounds turned to zero.
    """
    rxn_set = set()
...
def delete_model_genes(
    ...
    .. deprecated :: 0.25
        Use Functions.externals.MaCSBio_cobrapy.manipulation.knock_out_model_genes to simulate knockouts
        and Functions.externals.MaCSBio_cobrapy.manipulation.remove_genes to remove genes from
        the model.
    ...
    logger.warning(
        ...
        "Use Functions.externals.MaCSBio_cobrapy.manipulation.remove_genes instead to remove genes "
        "from the model."
    )
    logger.warning(
        f"Use Functions.externals.MaCSBio_cobrapy.manipulation.knock_out_model_genes({model}, {gene_list})"
...
class _GeneRemover(NodeTransformer):
    ...
    Parameters
    ----------
    target_genes: list or set of Functions.externals.MaCSBio_cobrapy.Gene
    ...
    def visit_Name(self, node: "Name") -> Optional["Name"]:
        ...
        Returns
        -------
        Functions.externals.MaCSBio_cobrapy.Gene or None
...
def remove_genes(
    ...
    gene_list: list of Functions.externals.MaCSBio_cobrapy.Gene or gene ids
