...
import Functions.externals.MaCSBio_cobrapy as cobra
...
def read_sbml_model(
    ...
    **kwargs,
    ...
    filename : path to SBML file, or SBML string, or SBML file handle
        SBML which is read into Functions.externals.MaCSBio_cobrapy model
    ...
    **kwargs:
    ...
    Returns
    -------
    Functions.externals.MaCSBio_cobrapy.core.Model
    ...
    try:
        ...
        cobra_error = CobraSBMLError(
            ...
            "the `Functions.externals.MaCSBio_cobrapy.io.sbml.validate_sbml_model` function or via the "
...
def _sbml_to_model(
    ...
    **kwargs,
    ...
    """Create Functions.externals.MaCSBio_cobrapy model from SBMLDocument.
...
def write_sbml_model(
    ...
    **kwargs,
    ...
    """Write Functions.externals.MaCSBio_cobrapy model to filename.
    ...
    Parameters
    ----------
    cobra_model : Functions.externals.MaCSBio_cobrapy.core.Model
...
def _model_to_sbml(
    ...
    """Convert Functions.externals.MaCSBio_cobrapy model to SBMLDocument.

    Parameters
    ----------
    cobra_model : Functions.externals.MaCSBio_cobrapy.core.Model
        Functions.externals.MaCSBio_cobrapy model instance
...
def _create_bound(
    ...
    model : libsbml.Model
        SBML model instance
    reaction : Functions.externals.MaCSBio_cobrapy.core.Reaction
        Functions.externals.MaCSBio_cobrapy reaction instance from which the bounds are read.
...
def _create_parameter(
    ...
    sbo: str, optional
        SBO term for parameter. In Functions.externals.MaCSBio_cobrapy, it seems to be SBO_FLUX_BOUND ( "SBO:0000625")
...
def _parse_notes_dict(sbase) -> dict:
    """Create dictionary of Functions.externals.MaCSBio_cobrapy notes.
...
def _sbase_notes_dict(sbase: libsbml.SBase, notes: dict) -> None:
    ...
    notes : dict
        Notes information from Functions.externals.MaCSBio_cobrapy object.
    """
    if notes and len(notes) > 0:
        tokens = (
...
# -----------------------------------------------------------------------------
"""
Functions.externals.MaCSBio_cobrapy annotations will be dictionaries of the form:
    object.annotation = {
...
def _parse_annotations(sbase: libsbml.SBase) -> dict:
    """Parse Functions.externals.MaCSBio_cobrapy annotations from a given SBase object.
...
def _sbase_annotations(sbase: libsbml.SBase, annotation: dict) -> None:
    """Set SBase annotations based on Functions.externals.MaCSBio_cobrapy annotations.

    Parameters
    ----------
    sbase : libsbml.SBase
        SBML object to annotate
    annotation : dict, Functions.externals.MaCSBio_cobrapy annotation structure
        Functions.externals.MaCSBio_cobrapy object with annotation information
...
def validate_sbml_model(
    ...
    **kwargs:
    ...
    -------
    (model, errors)
    model : :class:`~Functions.externals.MaCSBio_cobrapy.core.Model.Model` object
        The Functions.externals.MaCSBio_cobrapy model if the file could be read successfully or None
        otherwise.
    errors : dict
    ...
    for key in ["COBRA_FATAL", "COBRA_ERROR"]:
        if len(errors[key]) > 0:
            LOGGER.error("Functions.externals.MaCSBio_cobrapy errors in validation, check error log for details.")
            break
    for key in ["COBRA_WARNING", "COBRA_CHECK"]:
        if len(errors[key]) > 0:
            LOGGER.error("Functions.externals.MaCSBio_cobrapy warnings in validation, check error log for details.")
            break
