...
if TYPE_CHECKING:
    ...
    from Functions.externals.MaCSBio_cobrapy import Model, Solution
    from Functions.externals.MaCSBio_cobrapy.summary import ReactionSummary
    import pandas as pd
...
class Reaction(Object):
    """Define the Functions.externals.MaCSBio_cobrapy.Reaction class.

    Reaction is a class for holding information regarding
    a biochemical reaction in a Functions.externals.MaCSBio_cobrapy.Model object.

    Reactions are by default irreversible with bounds
    `(0.0, Functions.externals.MaCSBio_cobrapy.Configuration().upper_bound)`
    ...
    `(Functions.externals.MaCSBio_cobrapy.Configuration().lower_bound, Functions.externals.MaCSBio_cobrapy.Configuration().upper_bound)`.
    ...
    **kwargs:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # The Functions.externals.MaCSBio_cobrapy.Genes that are used to catalyze the reaction
        self._genes = set()
        ...
        # self.model is None or refers to the Functions.externals.MaCSBio_cobrapy.Model that
        # contains self
        self._model = None
    ...
    def metabolites(self) -> Dict[Metabolite, float]:
        ...
        metaoblites: Dict[Metabolite, float]
            A copy of self._metabolites, which is a dictionary of Functions.externals.MaCSBio_cobrapy.Metabolite for
    ...
    def gpr(self) -> GPR:
        ...
        -------
        gpr: GPR
            The GPR class, see Functions.externals.MaCSBio_cobrapy.core.gene.GPR() for details.
        """
        return self._gpr
    ...
    def gpr(self, value: GPR) -> None:
        ...
        See Also
        --------
        Functions.externals.MaCSBio_cobrapy.core.gene.GPR()
        update_genes_from_gpr()
        """
        self._gpr = value
        self.update_genes_from_gpr()
    ...
    def model(self) -> Optional["Model"]:
        ...
        Returns
        -------
        model: Functions.externals.MaCSBio_cobrapy.Model, optional
    ...
    def __setstate__(self, state: Dict) -> None:
        ...
        Probably not necessary to set _model as the Functions.externals.MaCSBio_cobrapy.Model that
    ...
    def __add__(self, other: "Reaction") -> "Reaction":
        ...
        Parameters
        ----------
        other: Functions.externals.MaCSBio_cobrapy.Reaction
    ...
    def get_coefficient(self, metabolite_id: Union[str, Metabolite]) -> float:
        ...
        Parameters
        ----------
        metabolite_id : str or Functions.externals.MaCSBio_cobrapy.Metabolite
    ...
    def get_coefficients(
        ...
        metabolite_ids : iterable
            Containing ``str`` or ``Functions.externals.MaCSBio_cobrapy.Metabolite``s.
    ...
    def check_charge_balance(self) -> Dict[str, float]:
        """Compute charge balance for the reaction.
        ...
        # filter out 0 values
        return {k: v for k, v in reaction_element_dict.items()}

    def check_mass_charge_balance(self) -> Dict[str, float]:
        ...
        ValueError
            No elements were found in metabolite.
        ###  2025/02/18 Jelle Bonthuis added:
            This function along with: check_mass_balance and check_charge_balance
            have been added to separate the functionality of these functions.
        """
        reaction_element_dict = defaultdict(int)
        for metabolite, coefficient in self._metabolites.items():
            if metabolite.charge is not None:
    ...
    def check_mass_balance(self) -> Dict[str, float]:
        ...
        ValueError
            No elements were found in metabolite.
        ### 2025/02/18 Jelle Bonthuis added:
            Fix for correctly identifying superoxides as oxygen species in the mass balance.
            Without this fix many reactions would be incorrectly identified as unbalanced.
        ###  2025/02/18 Jelle Bonthuis added:
            This function should only check mass balance as the name suggets, it is only
            called in validate.check_mass_balance() which does not describe charge balance.
            Therefor the functionality of these are separated into three functions:
                - check_mass_balance
                - check_charge_balance
                - check_mass_charge_balance
        """
        reaction_element_dict = defaultdict(int)
        for metabolite, coefficient in self._metabolites.items():
            if metabolite.elements is None:
    ...
    def _associate_gene(self, cobra_gene: Gene) -> None:
        """Associates a Functions.externals.MaCSBio_cobrapy.Gene object with a Functions.externals.MaCSBio_cobrapy.Reaction.

        Parameters
        ----------
        cobra_gene : Functions.externals.MaCSBio_cobrapy.core.Gene.Gene
    ...
    def _dissociate_gene(self, cobra_gene: Gene) -> None:
        """Dissociates a Functions.externals.MaCSBio_cobrapy.Gene object with a Functions.externals.MaCSBio_cobrapy.Reaction.
    ...
    def summary(
        ...
        Returns
        -------
        Functions.externals.MaCSBio_cobrapy.summary.ReactionSummary
