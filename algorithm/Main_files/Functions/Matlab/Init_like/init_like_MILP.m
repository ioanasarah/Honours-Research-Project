function solution = init_like_MILP(model, expressionRxns, epsilon, rev2irrev);
...
A = sparse(ind, j, v, size(S, 1) +  5* length(edgeIndices) , reactionLength + 2 * size(S, 2));
[m,n,s] = find(S);
for i = 1:length(m)
    A(m(i),n(i)) = s(i);
end
...
expressionRxns(expressionRxns == -1) = median(expressionRxns(expressionRxns~=-1));
c_y = 1./expressionRxns; % don't maximize y
c_y(c_y == -1) = 1./median(expressionRxns(expressionRxns~=-1));
...
%b_t_M_LB = 0;
%b_t_M_UB = M;
b = [b_s;lb_indices;b_Yprime;b_yprime_LB;b_yprime_UB; reversiblePlus;];
%
...
%csense15(1) = 'G'; % t >= 0
%csense16(1) = 'L'; % t <= M
csense = [csense1 csense2  csense5 csense10 csense11 csense12 ]';
...
% selectedRows = round(abs(solution.cont(1:reactionLength)),6)>= (epsilon-(espilon/100));
