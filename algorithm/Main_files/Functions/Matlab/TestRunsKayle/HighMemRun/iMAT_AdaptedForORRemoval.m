function [tissueModel,solution] = iMAT_AdaptedForORRemoval(model, expressionRxns, threshold_lb, threshold_ub, TurnOnLoopConstraints, tol, core, logfile, runtime, epsilon)
...
if nargin < 8 || isempty(logfile)
    logfile = 'IMAT_MILPlog.txt';
end
if nargin < 7 || isempty(tol)
    tol = 1e-8;
end
if nargin < 6 || isempty(core)
    core={};
end

    
    RHindex = find(expressionRxns >= threshold_ub);
    RLindex = find(expressionRxns < threshold_lb); %% EDITED IN THIS VERSION OF IMAT SO THAT EXPRESSIONLESS REACTIONS ARE INCLUDED
    %RLindex = find(expressionRxns > 0 & expressionRxns < threshold_lb); % OLD VERISON
