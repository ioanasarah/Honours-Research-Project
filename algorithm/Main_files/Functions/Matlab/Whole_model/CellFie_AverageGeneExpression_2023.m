...
% %%%
% sample_1 = ExpValueAdjusted(:,1);
% rxnFormulas = printRxnFormula(model,model.rxns,printFlag );
% 
% printFlag = false;
% b = printRxnFormula(model,model.rxns,printFlag );
% 
% pattern = '(\d+(?:\.\d+)?)?\s?MAM\d+\[[a-z]\]';
% pattern3 = 'MAM\d+\[[a-z]\]';
% pattern2 = '\[(.*?)\]';
% reversible_arrow = '<=>';
% forward_arrow = '->';
...
%     fullFormula = "";
%     if contains(c, reversible_arrow)
%         % Reaction contains a reversible arrow
%         splitString = split(c, reversible_arrow);
%     elseif contains(c, forward_arrow)
%         % Reaction contains an irreversible forward arrow
%         splitString = split(c, forward_arrow);
%     elseif contains(c, backward_arrow)
%         % Reaction contains an irreversible backward arrow
%         splitString = split(c, backward_arrow);
%     else
%         error('Reaction string does not contain a valid arrow notation');
%     end
% 
%     %splitString = split(c, '->');
% 
...
%         idx = findMetIDs(model,metabolitesBeforeForIdx(cIterator));
%         metName = model.metNames(idx);
%         numberStr = regexprep(metabolitesBefore{cIterator}, '(\d+(?:\.\d+)?)?\s?MAM\d+\[[a-z]\]', '$1');
...
%         idx = findMetIDs(model,metabolitesAfterForIDX(cIterator));
%         metName = model.metNames(idx);
%         numberStr = regexprep(metabolitesAfter{cIterator}, '(\d+(?:\.\d+)?)?\s?MAM\d+\[[a-z]\]', '$1');
%         numberOfMetabolites = str2double(numberStr);
...
% 
% 
% columnNames = {'reaction_IDs', 'Expression_sample1', 'Full name', 'Formula' ,'Lower bound', 'Upper bound',  'Subsystem', 'Rules','Gene-regulatory rules', 'References'};
% subSystems = model.subSystems;
% numVectors = length(subSystems);  % Number of 2x1 vectors in subSystems
% concatenatedStrings = strings(numVectors, 1);  % Pre-allocate string array
% 
% 
% 
% % Iterate through the cell array and concatenate the elements of each vector
% for i = 1:numVectors
%     % Access the 2x1 vector from subSystems
%     vector = subSystems{i};
%     if length(vector)>1
%         new_string = string(vector(1));
%         for j = 2:length(vector)
%             new_string = new_string + " And " + string(vector(j));
%         end
%         subSystems{i} = new_string;
%     end
% end
% 
% convertedSubsystem = string(subSystems);
% new_data = table(model.rxns, sample_1, model.rxnNames, rxnFormulas, model.lb, model.ub, convertedSubsystem, model.rules, model.grRules, model.rxnReferences, 'VariableNames', columnNames);
% writetable(new_data, 'model_with_expression_sample1.xlsx');
% %%%
% 
% 
...
for i=332:size(taskStructure,1)%, 1)%param.doParallel)
    ...
    for j=1:1%SampleNumber % For testing, adjust to 1 or whatever sample number you want
        ...
        if PassTask
            ...
            matches = contains(newIrrevModelKapprox.rxnNames, 'Generic human cell biomass reaction', 'IgnoreCase', true);
            matchingIndices = find(matches);
            newIrrevModelKapprox.lb(matchingIndices) = 0.01;
            newIrrevModelKapprox.lb(findRxnIDs(newIrrevModelKapprox, 'MAR03964')) = 0.01; % 
            newIrrevModelKapprox.ub(matchingIndices)
            newIrrevModelKapprox.ub(findRxnIDs(newIrrevModelKapprox, 'MAR03964')) % 
            optimizeCbModel(newIrrevModelKapprox)

            newIrrevModelKapprox.rxnNames(matchingIndices)
