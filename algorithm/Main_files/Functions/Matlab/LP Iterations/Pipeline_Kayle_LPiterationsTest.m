...
for iTask = 1:1 %size(TaskStructure,1)  % Choose which task(s) will run
    ...
    for jSample = 1:1 %nSamples         % For testing, adjust to 1 or whatever sample number you want
        ...
        if passTask
            ...
            fprintf("\nStart LP iterations Task %i Sample %i\n\nAlpha = %0.2f\nBeta = %0.2f\nEpsilon = %0.2f\n",iTask,jSample,alpha,beta,epsilon)

            orphanHandleParameter = median(expValuesMeanAdjustedTaskIrrevKapprox(expValuesMeanAdjustedTaskIrrevKapprox>0));
            expValuesMeanAdjustedTaskIrrevKapprox(expValuesMeanAdjustedTaskIrrevKapprox == -1) = orphanHandleParameter;
            
            [kfeasibletest, optionsSaved] =  findHighestKIterativeLPiterationsTest(NewIrrevModelKapprox,expValuesMeanAdjustedTaskIrrevKapprox,osenseStr);
