function taskStruct=generateTaskStructure_2022_Jelle(inputFile)
...
%    - Modified by BSC Nihant (2022)
%    - Modified by Jelle (for irreversible reactions and set minimum flux to epsilon = 0.5
%    
epsilon = 1;% Modified by Jelle 
...
columns={'ID';'DESCRIPTION';'IN';'IN LB';'IN UB';'OUT';'OUT LB';'OUT UB';'SHOULD FAIL';'SYSTEM';'SUBSYSTEM';'EQU';'EQU LB';'EQU UB'};
nan = false(size(raw(1,1:end)));
for i = 1:length(nan)
    nan(i) = sum(isnan(raw{1,i}));
end
raw = raw(:, ~nan);
...
for i=2:size(raw,1)
    ...
    if ischar(raw{i,colI(12)})
        task.EQU=[task.EQU;raw{i,12}];
        task.EQULB=[task.EQULB;raw{i,colI(13)}];
        task.EQUUB=[task.EQUUB;raw{i,colI(14)}];
    end
