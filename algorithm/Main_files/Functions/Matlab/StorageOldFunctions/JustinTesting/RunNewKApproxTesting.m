...
for i= testTasks%size(taskStructure,1)%, 1)%param.doParallel)
    ...
    for j=1:1%SampleNumber % For testing, adjust to 1 or whatever sample number you want
        ...
        if PassTask
            ...
            FolderNameRun = sprintf("Task%iSample%iTestingFastCCAlpha%0.2fBeta%0.2f_Date%s",i,j,alpha, beta,currentDateTimeFolder);
            mkdir(FolderNameRun)
            copyfile('convertToEscherModelWithInputs.py', FolderNameRun)
            cd(FolderNameRun)
            diary logFile.txt

            changedValue = [0.001, true, false];
            for el = changedValue
                if el not false
                    if el == 0.001 
                        tic
                        [ATask, fastCCModel, V] = fastcc(taskModelOldCore, el);
                        toc 
                        name = "RunNewKApproxHTv2_homeSettingsFastCC0.001";
                    elseif el == true
                        % Default
                        tic
                        [ATask, fastCCModel, V] = fastcc(taskModelOldCore);
                        toc 
                        name = "RunNewKApproxHTv2_homeSettingsFastCCdefault";
                    end
                    ...
                    newReversibleModelForKApproxmiation = taskModelOldCore;
                    expValueMeanAdjustedTaskReversibleKApproximation = expValueMeanAdjustedTaskOld;
                    name = "RunNewKApproxHTv2_homeSettingsNoFastCC";
                end
                ...
                currentDateTimeFolder = datestr(now, 'yyyy-mm-dd_HH-MM-SS');
                FolderNameRun1 = sprintf("%s_%s", name, currentDateTimeFolder);
                mkdir(FolderNameRun1)
                copyfile('convertToEscherModelWithInputs.py', FolderNameRun1)
                cd(FolderNameRun1)
                ...
                %filenameexpValueMeAdjustedTaskIrrevKapprox = sprintf('LastCreatedexpValueMeanAdjustedTaskIrrevKapprox_Task%iSample%i_date_%s.mat', i,j,currentDateTime);
                ...
                cd("/home/jcornelis/Documents/MaCSBio-GEM-GIT/General/Functions/Metabolic_tasks/Jelle/JuTest/MILPs/" + FolderNameRun)
            end
