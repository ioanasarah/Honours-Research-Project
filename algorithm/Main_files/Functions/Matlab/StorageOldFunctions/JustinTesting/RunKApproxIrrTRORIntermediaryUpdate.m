...
for i=1:1%size(taskStructure,1)%, 1)%param.doParallel)
    ...
    for j=1:1%SampleNumber % For testing, adjust to 1 or whatever sample number you want
        ...
        if PassTask
            ...
            %%% TROR adjustment code
            alpha = 0.2; % even addition of the TROR adjustment as regular obj funciton at 0.5
            % alpha gives the weights to the TROR adjustment of the obj
            % function and to the original objective function by setting
            beta = 10;  % total addition to max score if all oprhan transport reactions would be 0

            [solutionIrrevFilterLoopLawKapproxTROR_Adjustment,scoreIrrevFilterLoopLawKapproxTROR_Adjustment,reactionsToKeepIrrevFilterLoopLawKapproxTROR_Adjustment] = runKApproxmitationIrrevTROR_AdjustmentIntermediaryUpdate (newIrrevModelKapprox, expValueMeanAdjustedTaskIrrevKapprox, epsilon, rev2irrevIrrevKapprox, alpha, beta, i, j);
