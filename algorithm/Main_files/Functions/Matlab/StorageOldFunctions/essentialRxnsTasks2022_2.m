function [essentialRxns,usedRxns] = essentialRxnsTasks2022_2(model,saveUsed)
...
% USAGE:
%
%    [essentialRxns,usedRxns] = essentialRxnsTasks(mode,saveUsedl)
%
% INPUT:
...
if ~exist('saveUsed','var')
    saveUsed = false;
end
	[solMin, modelIrrevFMOri]= minimizeModelFlux(model,'min'); %Compute the minimal set of reactions
	modelIrrevFMOri = changeRxnBounds(modelIrrevFMOri,'netFlux',solMin.f + 1e-06,'u');
    modelIrrevFMOri = changeRxnBounds(modelIrrevFMOri,'netFlux',solMin.f - 1e-06,'l');
