function [newSolution, prunedModel, originalModel, adjustedExpressionData, LogData ] = runRxnsMinimalizationUsingIMAT(taskModel, solution, expValueMeanAdjustedTask,reactionLength, epsilon,TurnOnLoopConstraints)
...
logicalIndicesExpressionNotInSolution = find((solution.int(1:reactionLength) == 0) & (expressionIndices == 1));
...
taskModel1 = changeRxnBounds(taskModel, taskModel.rxns(logicalIndicesExpression1), epsilon, 'l');
taskModel1 = changeRxnBounds(taskModel1, taskModel1.rxns(logicalIndicesExpressionNotInSolution), 0);
...
[prunedModel,newSolution] = iMAT_AdaptedForORRemoval(taskModel1, expValueMeanAdjustedTask, threshold_lb, threshold_ub,TurnOnLoopConstraints);
...
LogData = "\nFaulty iMAT results";
numberReactionsModelOriginal = size(originalModel.rxns,1);
numberReactionsModelPruned = size(prunedModel.rxns, 1);
...
LogData = LogData + sprintf("Amount of reactions in original and pruned model: %i -- %i \n",numberReactionsModelOriginal, numberReactionsModelPruned);

if length(logicalIndicesExpression1) ~= length(find((newSolution.int(1:reactionLength) == 0) & (expressionIndices == 1)))
