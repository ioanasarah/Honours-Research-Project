function MILPproblem = createKMILPProblemAverageExpression(model, expressionRxns, epsilon, rev2irrev,k )
tic
...
A = sparse(ind, j, v, size(S, 1) +  6* length(edgeIndices) + 1, reactionLength + 3 * size(S, 2));
[m,n,s] = find(S);
for i = 1:length(m)
    A(m(i),n(i)) = s(i);
end
...
lb = [lb;lb_y;lb_y;lb_y];
ub = [ub;ub_y;ub_y;ub_y];
...
%b_t_M_LB = 0;
%b_t_M_UB = M;
b = [b_s;lb_indices;ub_indices;b_Yprime;b_yprime_LB;b_yprime_UB; reversiblePlus;b_AverageExpressRow;];
%
...
%csense15(1) = 'G'; % t >= 0
%csense16(1) = 'L'; % t <= M
csense = [csense1 csense2 csense3  csense5 csense10 csense11 csense12 csense14];
