function [solution,score,reactionsToKeep] = runKApproxmitationAdaptedNoYMinusWithLoopLaw (taskModel, expValueMeanAdjustedTask, epsilon, rev2irrev)
...
MILPproblem = createKMILPProblemAverageExpressionNoYiMinusLoopLaw(taskModel, expValueMeanAdjustedTask, epsilon, rev2irrev, ktest);
...
while abs(kfeasibletest - kInfeasibletest) > kTolerance
    ...
    disp(length(find(solution.cont>0.99)))
end 
