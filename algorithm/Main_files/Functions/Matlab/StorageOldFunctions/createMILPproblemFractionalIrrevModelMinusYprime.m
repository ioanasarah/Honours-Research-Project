function MILPproblem = createMILPproblemFractionalIrrevModelMinusYprime(model, expressionRxns, epsilon,TurnOnLoopConstraints, rev2irrev )
...
for i = 1:length(edgeIndices)
    ...
    count = count + 1;
    ind(count) = i+size(S,1)+14*length(edgeIndices);
    j(count) = i+ 1*size(S,2);
    v(count) = 1;
    ...
    count = count + 1;
    ind(count) = i+size(S,1)+15*length(edgeIndices);
    j(count) = i+ 2*size(S,2);
    v(count) =  1;
...
A = sparse(ind, j, v, size(S, 1) + 16 * length(edgeIndices) + 3, reactionLength + 5 * size(S, 2) + 1);
[m,n,s] = find(S);
for i = 1:length(m)
    A(m(i),n(i)) = s(i);
end
...
b_yprime_LB = zeros(size(S,2),1);  % Yprime >= LB
b_yprime_UB = zeros(size(S,2),1); % Yprime  >= UB
reversiblePlus =  ones(size(S,2),1); % link reversible reaction <=1
reversibleMinus = ones(size(S,2),1); % link reversible reaction <=1
b_z = 1;
b_t_M_LB = 0;
b_t_M_UB = M;
b = [b_s;lb_indices;ub_indices;b_YPlusY;b_Yprime;b_z1; b_z2 ;b_z3; b_z4; b_z1; b_z2; b_z3; b_z4; b_yprime_LB;b_yprime_UB; reversiblePlus;reversibleMinus; b_z;b_t_M_LB;b_t_M_UB];
%
...
csense15(1) = 'G'; % t >= 0
csense16(1) = 'L'; % t <= M
csense = [csense1 csense2 csense3 csense4 csense5 csense6 csense7 csense8 csense9 csense6 csense7 csense8 csense9 csense10 csense11 csense12 csense13 csense14 csense15 csense16];
