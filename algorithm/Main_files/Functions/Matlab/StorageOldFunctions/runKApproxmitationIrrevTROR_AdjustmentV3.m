function [solution,score,reactionsToKeep] = runKApproxmitationIrrevTROR_AdjustmentV3 (taskModel, expValueMeanAdjustedTask, epsilon, rev2irrev, alpha, beta, i,j,osenseStr,irrev,orphanHandling)
...
kInfeasibletest =  ((2*alpha)* max(expValueMeanAdjustedTask)) + (2*(1-alpha)*beta) + TransportReactionPenalty;
counter = 0;
...
timeLimit_ConstantInitialInfeas = 240;                       
...
fprintf("kFeasible = %.3f ___ kInfeasible = %.3f ___ TransportReactionPenalty = %i \n",kFeasible,kInfeasible,TransportReactionPenalty)
ktest = kInfeasibletest;
kdiff = abs(kfeasibletest - kInfeasibletest);
infeasibleCounter = 0;
while ~ExceededInitialInfeasConstraint && ~noWeirdFeasibleSolutionsInInfeasibleSpace && kdiff >= kTolerance
    infeasibleCounter = infeasibleCounter+1; 
    varargin.timeLimit = timeLimit_ConstantInitialInfeas;
    ...
    logFile = sprintf("Infeasible_Space_run_%i.txt",infeasibleCounter);
    ...
    if solution.stat == 1|| solution.stat == 10 % Optimal (1) or feasible (10) found 
        ...
        kfeasibletest = ktest;
        noWeirdFeasibleSolutionsInInfeasibleSpace = true; 
        currentDateTime = datestr(now, 'yyyy-mm-dd___HH-MM-SS');
...
timeLimit_ConstantFirstFeas = 600;
...
if UseSpecialYflags
    ...
    for i = 1:reactionLength
        if(expValueMeanAdjustedTask(i) ~= -1)
            MILPproblem.c(reactionLength+i) = alpha*2*(expValueMeanAdjustedTask(i)-ktest); %*2 is there since 0.5 is alpha default value
        else
            MILPproblem.c(reactionLength+i) = 0; %*2 is there since 0.5 is alpha default value
        end
        if(expValueMeanAdjustedTask(i) == -1)
            MILPproblem.c(2*reactionLength+i) = ((1-alpha)*2*betaAdjusted); %*2 is there since 0.5 is alpha default value
        else
            MILPproblem.c(2*reactionLength+i)= 0;
        end
    end
...
while (~approximatedOptimalSolutionThroughTimeHeuristic && ~couldNotFindAnySolution && kdiff >= kTolerance)
    ...
    timerStart = tic;
    rng(1) % unfortunately only gurobi method 4 and 5 are deterministic
    logFile = sprintf("feasible_Space_run_%i.txt",infeasibleCounter);
    varargin.logFile = logFile;
    solution = solveCobraMILP_Adjusted(MILPproblem,varargin);
    
    if solution.stat == 1|| solution.stat == 10 % Optimal (1) or feasible (10) found     
        kfeasibletest = findTrueKValue(solution, MILPproblem, expValueMeanAdjustedTask, alpha, beta,reactionLength,ktest,SMatrix);
...
if exist('lastValidSolution')
    ...
    score = findTrueKValue(solution, MILPproblem, expValueMeanAdjustedTask, alpha, beta,reactionLength,ktest,SMatrix);
