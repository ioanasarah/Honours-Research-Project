...
expressionRxns(oxphosRxnIds) = 10;

% Test reactions for alternative flux
testRxnIds = findRxnIDs(Oldmodel,{'EX_acald_e', 'ACALD','ACALDt'});
expressionRxns(testRxnIds) = 0.000000001;
%%
...
A_ColumnLength = size(S,2) + YAmount*reactionLength + ZAmount*reactionLength + 2*reactionLength  + TAmount;
...
for i = 1:length(edgeIndices)
    ...
    A(size(S,1)+14*length(edgeIndices) + 1, i + 6*size(S,2)) = 1;
...
lb = [lb;lb_y;lb_y;lb_y;lb_z;lb_z;lb_z;lb_t];
ub = [ub;ub_y;ub_y;ub_y;ub_z;ub_z;ub_z;ub_t];

% Creating vartype (equal to column length:
% Column length equals S matrix + 2Y*rxn + 2z*rxn + 1*t )
vartype_v(1:size(S,2),1) = 'C'; % Vi
vartype_y(1:3*reactionLength,1) = 'B'; %Yi + - 
vartype_z(1:3*reactionLength,1) = 'C'; %Zi + -
vartype_t(1,1) = 'C'; % t
...
c = [c_v;c_y;c_y;c_yprime;c_z;c_z;c_zprime;c_t];
...
ReactionSolutions(1:reactionLength,5) = expressionRxns;
...
activeReactions = ReactionSolutions([ReactionSolutions{:, 3}] ~= 0, :);
