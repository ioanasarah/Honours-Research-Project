function solution = solveCobraMILP_Adjusted(MILPproblem, varargin)
...
switch solver
    ...
    case 'gurobi'
        ...
        if exist('x0hint') && ~isempty(x0hint)
        ...
        elseif strcmp(resultgurobi.status, 'SOLUTION_LIMIT')
            solStat = 10;
            try 
                [x,f,xn] = deal(resultgurobi.x,resultgurobi.objval, resultgurobi.xn);
            catch
                try
                    [x,f]= deal(resultgurobi.x,resultgurobi.objval);
                catch
                    try
                        [x] = deal(resultgurobi.x);
                    catch
                    end
                end
            end
        elseif strcmp(resultgurobi.status, 'INTERRUPTED')
            solStat = 11;
            try
                [x,f,xn] = deal(resultgurobi.x,resultgurobi.objval, resultgurobi.xn);
            catch
