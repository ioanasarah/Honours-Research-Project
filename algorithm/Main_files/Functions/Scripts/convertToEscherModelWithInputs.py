...
## mat_files_directory = 'E:/Git/MaCSBio-GEM/General/Functions/Metabolic_tasks/Task1Sample1Alpha0.50Beta5.00_Date2023-10-22_13-54-20'
...
logFiles_directory = os.path.join(mat_files_directory, 'logFiles')
os.makedirs(logFiles_directory, exist_ok=True)
...
# Find all newModel .mat files
file_list = [file for file in os.listdir(mat_files_directory) if file.startswith('CharnesCooperSolution') and file.endswith('.mat')]
for file in file_list:
    old_path = os.path.join(mat_files_directory, file)
    new_path = os.path.join(LastValidSOlutions_directory, file)
    os.rename(old_path, new_path)
...
file_list = [file for file in os.listdir(mat_files_directory) if file.endswith('.txt')]
for file in file_list:
    old_path = os.path.join(mat_files_directory, file)
    new_path = os.path.join(logFiles_directory, file)
    os.rename(old_path, new_path)
