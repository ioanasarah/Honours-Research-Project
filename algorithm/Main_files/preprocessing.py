...
This module contains the preprocessing functions that will take the following inputs:
    ...
    - A tasklist (in the form of a json or excel file) with the following characteristics:
        ...
        - "IN" as the fourth column with the input metabolites in MAMxxxxx notation
        ...
        - "OUT" as the seventh column with the output metabolites in MAMxxxxx notation
            ...
            - Specific naming convention for the different methods of creating the expression data:
                ...
                - GPR_method:
                                    ...
                                    trimmin2Max_V (taking the min of AND values after trimming V%),
                                                and then taking the maximum of OR values
                - should_filter_potential_trim_genes:  True (genes that could potentially be trimmed will
                                                            undergo a filtering step to ensure that variable
                                                            and thus likely biologically relevant will not
                                                            be trimmable)
                                                       False
                - trim_filter_flat_reduction:  The method utilizes a flat reduction in its calculation
                                                        in order to bias the trimming ratio
                                                        threshold to very lowly expressed genes
                - trim_filter_percentiles:     The percentiles that are used in the trimming filter
                                                        specifically used to choose the samples per gene
                                                        that will be compared to find an M-value.
                - trim_filter_threshold        The threshold that is used to determine if a gene is
                                                        a potential candidate for trimming.
                                                        Specifically the M-value of the gene has to
                                                        be smaller than this threshold
            "promiscuity_method": "N",

                - promiscuity_method:   Y -> Currently not implemented as of version 1.05
                                                (yes, a gene being the determining factor
                                                 for multiple reactions is reduced by 1/n)
                                        N (no)
...
from Functions.externals._cobra import (load_matlab_model, save_json_model)
import time
...
import re
import pyarrow
from CodeVersion import version
import warnings
version = "1.0.4"
'''
...
class FileChecker:
    ...
    def read_model_file_in_folder(
        ...
        if model_file_name is not None:
            try:
                ...
                warning(f"An error occurred while reading the model file {model_file}: {e}")
        ...
        for model_file in model_files:
            try:
                ...
                warning(f"An error occurred while reading the model file {model_file_name}: {e}")
            # except Exception:
            #     pass
            except Exception as e:
                pass
    ...
    def raise_warning_if_origin_files_not_present(
        ...
        if not os.path.exists(file_or_folder_path):
            warning(
                f"The combination folder is present and populated, but the origin file or folder {file_or_folder_path} does not exist.\n"
    ...
    def check_if_combination_files_exist(
        ...
        irreverible_or_reversible = "irreversible" if global_options_dict["__global_options"][
                                                          "model_type"] == "irr" else "reversible"
    ...
    def check_if_combination_model_is_correct(
        ...
        if not all(reaction.upper_bound is not None and reaction.lower_bound is not None for reaction in
                   cobra_model.reactions):
            warning(f"The model file {model_file_name} contains reactions without bounds.")
            is_correct = False
        if any(rxn.lower_bound != 0 and rxn.upper_bound != 0 and (
                (rxn.upper_bound > 0 and rxn.lower_bound > 0) or (rxn.upper_bound < 0 and rxn.lower_bound < 0)) for rxn
    ...
    def check_if_irrev_dict_is_correct(
        ...
        try:
            ...
            warning(
                f"An error occurred while reading the irreversible dictionary {rev_to_irrev_dict_name_by_options}.\n"
                f"Error message: {e}")
            return False, None
        if not all(isinstance(key[0], int) for key in rev_to_irrev_dict) or not all(
                isinstance(key[1], int) for key in rev_to_irrev_dict if len(key) > 1):
        ...
        if not is_correct:
            ...
            os.rename(rev_to_irrev_dict_name_by_options,
                      rev_to_irrev_dict_name_by_options.split(".json")[0] + "_incorrect.json")
            return False, None
    ...
    def check_if_combination_taskstructure_is_correct(
        ...
        for sample_id, task in task_structure.items():
            if not isinstance(sample_id, int) and not isinstance(sample_id, str):
                warning(
                    f"The task_structure file {tasklist_file}, sample {sample_id} contains a sample ID that is not a string.")
                is_correct = False
                break
            if not isinstance(task, dict):
                warning(
                    f"The task_structure file {tasklist_file}, sample {sample_id} contains a task that is not a dictionary.")
                is_correct = False
                break
            if "task_id" not in task:
                warning(
                    f"The task_structure file {tasklist_file}, sample {sample_id} contains a task without a task ID.")
                is_correct = False
                break
            if "task_description" not in task:
                warning(
                    f"The task_structure file {tasklist_file}, sample {sample_id} contains a task without a task description.")
                is_correct = False
                break
            if "input_metabolites" not in task:
                warning(
                    f"The task_structure file {tasklist_file}, sample {sample_id} contains a task without input metabolites.")
                is_correct = False
                break
            if "input_metabolite_lower_bounds" not in task:
                warning(
                    f"The task_structure file {tasklist_file}, sample {sample_id} contains a task without input metabolite lower bounds.")
                is_correct = False
                break
            if "input_metabolite_upper_bounds" not in task:
                warning(
                    f"The task_structure file {tasklist_file}, sample {sample_id} contains a task without input metabolite upper bounds.")
                is_correct = False
                break
            if "output_metabolites" not in task:
                warning(
                    f"The task_structure file {tasklist_file}, sample {sample_id} contains a task without output metabolites.")
                is_correct = False
                break
            if "output_metabolite_lower_bounds" not in task:
                warning(
                    f"The task_structure file {tasklist_file}, sample {sample_id} contains a task without output metabolite lower bounds.")
                is_correct = False
                break
            if "output_metabolite_upper_bounds" not in task:
                warning(
                    f"The task_structure file {tasklist_file}, sample {sample_id} contains a task without output metabolite upper bounds.")
                is_correct = False
                break
        if not is_correct:
    ...
    def check_if_combination_files_are_correct(
        ...
        if not is_correct:
            check_if_all_correct_dict["expression_data"] = False
        (is_correct, cobra_model) = FileChecker.check_if_combination_model_is_correct(model_name_by_options,
                                                                                      cobra_model)
        if not is_correct:
            check_if_all_correct_dict["model"] = False
        ...
        (is_correct, tasklist) = FileChecker.check_if_combination_taskstructure_is_correct(tasklist_name_by_options,
                                                                                           tasklist)
        if not is_correct:
...
class CombinationFileCreation:
    ...
    def create_rev2irrev_dict(
        ...
        if CombinationFileCreation.check_if_model_is_irreversible(cobra_model):
            ...
            reaction_pair_indices = {idx: idx2 for idx in range(len(cobra_model.reactions)) for idx2, rxn in
                                     enumerate(cobra_model.reactions) if
                                     rxn.id == cobra_model.reactions[idx].id + "_r" or rxn.id == cobra_model.reactions[
                                         idx].id + "_b"}
            return reaction_pair_indices
        else:
            cobra_model, reaction_pair_indices = CombinationFileCreation.convert_reversible_model_to_irreversible(
                cobra_model)
            return reaction_pair_indices
        ...
        for reaction in cobra_model.reactions:
            ...
            if reaction.lower_bound != 0 and reaction.upper_bound != 0 and (
                ...
                if reaction.lower_bound < 0 and reaction.upper_bound < 0:
                    ...
                    reaction.upper_bound = 1000
        if irreversible:
            cobra_model, rev_to_irrev_dict = CombinationFileCreation.convert_reversible_model_to_irreversible(
                cobra_model)
        else:
            rev_to_irrev_dict = None
    ...
    #                     continue
    #                 if value == min_value:
    #                     gene_contribution_dict_input[key] = (
    #                             gene_contribution_dict_input[key] + 1) if key in gene_contribution_dict_input else 1
    #                 else:
    #                     gene_contribution_dict_input[key] = max(0, gene_contribution_dict_input[
    #                         key] - 1) if key in gene_contribution_dict_input else 0
    ...
    #                     gene_contribution_dict_input[key] = max(0, gene_contribution_dict_input[
    #                         key] - 1) if key in gene_contribution_dict_input else 0
    #         elif rule == "trimmean" or rule == "trimmin1" or rule == "trimmin2" or rule == "trimmax": # TODO make this similar to the working code of
    #             # TODO the not-in promiscuity code (some issue with filtering too much)
    ...
    #                         gene_contribution_dict_input[key] = (gene_contribution_dict_input[
    #                                                                  key] + 1) if key in gene_contribution_dict_input else 1
    #                     else:
    #                         gene_contribution_dict_input[key] = max(0, gene_contribution_dict_input[
    #                             key] - 1) if key in gene_contribution_dict_input else 0
    ...
    #             elif trim_count >= len(sorted_values) / 2:
    #                 trimmed_values = sorted_values[trim_count:len(sorted_values)]
    #             elif len(sorted_values) - trim_count <= 1:
    #                 trimmed_values = sorted_values[trim_count:len(sorted_values)]
    #             else:
    #                 trimmed_values = sorted_values[trim_count:len(sorted_values) - trim_count]
    #
    ...
    #
    # @staticmethod
    # def identify_and_allocate_promiscuity_processes(
    #         cobra_model: Model,
    #         amount_of_input_dissimilarity_to_still_be_subprocess: int = 1,
    #         amount_of_output_dissimilarity_to_still_be_subprocess: int = 1,
    #         care_about_compartment: bool = True,
    # ):
    #
    #     reaction_subprocess_designation_dict = {
    #         reaction.id: None for reaction in cobra_model.reactions
    #     }
    #     seen_gpr_rules = {}
    #     counter = 0
    #     reactions_per_potential_set = {}
    #     for reaction in cobra_model.reactions:
    #         if reaction.gene_reaction_rule != "":
    #             if reaction.gene_reaction_rule not in seen_gpr_rules:
    #                 seen_gpr_rules[reaction.gene_reaction_rule] = counter
    #                 reaction_subprocess_designation_dict[reaction.id] = counter
    #                 reactions_per_potential_set[counter] = [reaction]
    #                 counter += 1
    #             else:
    #                 reaction_subprocess_designation_dict[reaction.id] = seen_gpr_rules[reaction.gene_reaction_rule]
    #                 reactions_per_potential_set[seen_gpr_rules[reaction.gene_reaction_rule]].append(reaction)
    #
    #     for potential_set in reactions_per_potential_set.items():
    #         reactions_per_potential_set = potential_set
    #         break # TODO only for testing/editing
    #
    #
    #     for potential_set in reactions_per_potential_set.values():
    #         minimum_amount_of_subsets = len(potential_set)
    #         reactions_in_solution = potential_set.copy()
    #         reactions_that_could_be_in_set_together = {
    #             reaction: [] for reaction in potential_set
    #         }
    #         for reaction in potential_set:
    #             metabolites_in = [metabolite.id for metabolite in reaction.metabolites if
    #                               reaction.metabolites[metabolite] < 0]
    #             metabolites_out = [metabolite.id for metabolite in reaction.metabolites if
    #                                reaction.metabolites[metabolite] > 0]
    #             for reaction2 in potential_set:
    #                 if reaction2 == reaction:
    #                     continue
    #                 metabolites_in2 = [metabolite.id for metabolite in reaction2.metabolites if
    #                                    reaction2.metabolites[metabolite] < 0]
    #                 metabolites_out2 = [metabolite.id for metabolite in reaction2.metabolites if
    #                                     reaction2.metabolites[metabolite] > 0]
    #                 if len(set(metabolites_in).symmetric_difference(
    #                         set(metabolites_in2))) <= amount_of_input_dissimilarity_to_still_be_subprocess and (
    #                         len(set(metabolites_out).symmetric_difference(
    #                             set(metabolites_out2))) <= amount_of_output_dissimilarity_to_still_be_subprocess):
    #                     if care_about_compartment:
    #                         if not reaction.compartments == reaction2.compartments:
    #                             continue
    #                     reactions_that_could_be_in_set_together[reaction].append(reaction2)
    #
    #         # find allocation of subprocesses based on the reactions that could be in the same subprocess and that
    #         # would lead to the least amount of subprocesses
    #         # if there are no reactions that could be in the same subprocess, then the reaction is in a subprocess by itself
    #
    #         identical_or_empty_processes = []
    #         for reactions_together in reactions_that_could_be_in_set_together.values():
    #             if reactions_together not in identical_or_empty_processes:
    #                 identical_or_empty_processes.append(reactions_together)
    #
    #
    #
    #
    ...
    #     #     reactions_with_gene = gene.reactions
    #     #     groups_with_identical_GPR_rules = {}
    #     #     for reaction in reactions_with_gene:
    #     #         if reaction.gene_reaction_rule not in groups_with_identical_GPR_rules:
    #     #             groups_with_identical_GPR_rules[reaction.gene_reaction_rule] = [reaction]
    #     #         else:
    #     #             groups_with_identical_GPR_rules[reaction.gene_reaction_rule].append(reaction)
    #     #     for group in groups_with_identical_GPR_rules.values():
    #
    # @staticmethod
    ...
    #         operator, operands = parsed_expr
    #         values_dict = {
    #             CombinationFileCreation.create_string_from_parsed_expression(
    #                 op): CombinationFileCreation.evaluate_expression(
    ...
    #                 return values
    #             elif and_rule == "trimmin1" or and_rule == "trimmin2":
    ...
    #         operator, operands = parsed_expr
    #         for op in operands:
    #             if CombinationFileCreation.create_string_from_parsed_expression(
    #                     op) in gene_contribution_dict_in_reaction_input.keys():
    #                 if gene_contribution_dict_in_reaction_input[
    #                     CombinationFileCreation.create_string_from_parsed_expression(op)] > 0:
    #                     pass
    #         values_dict = {
    #             CombinationFileCreation.create_string_from_parsed_expression(
    #                 op): CombinationFileCreation.evaluate_expression_in_promiscuity(
    ...
    #                and gene_contribution_dict_in_reaction_input[
    #                    CombinationFileCreation.create_string_from_parsed_expression(op)] > 0
    #         }
    #         values_dict = {key: value for key, value in values_dict.items() if value is not None}
    ...
    #                 values = sum(values_dict.values()) / len(values_dict)
    #                 return values
    #             elif GPR_AND_argument == "trimmin1":
    #                 values = min(values_dict.values())
    #                 return values
    ...
    #             genes_in_reaction.append(
    #                 potential_genes[0]) if potential_genes and potential_genes not in genes_in_reaction else None
    #
    ...
    #     per_reaction_gene_contribution_tuple_matrix = [[{} for _ in range(sample_number)] for _ in
    #                                                    range(len(cobra_model.reactions))]
    #     per_reaction_adjusted_gene_contribution_tuple_matrix = [[{} for _ in range(sample_number)] for _ in
    #                                                             range(len(cobra_model.reactions))]
    ...
    #                 genes_in_reaction = rxn.genes
    #                 # expression_in_reaction = [gene_values[gene.id] for gene in genes_in_reaction]
    #                 # if GPR_AND_argument.startswith("trim"):
    #                 #     conservative_trim_count = int(len(expression_in_reaction) * GPR_value)
    #                 #     expression_in_reaction = sorted(expression_in_reaction)
    #                 #     expression_in_reaction = expression_in_reaction[conservative_trim_count:len(expression_in_reaction)]
    #
    #                     # TODO make it so that genes available for that reaction are only those that are not 0 contribution
    #                     # possibly by making a filtered gene values from which it can search
    #
    ...
    #                 gene_contribution_dict_in_reaction = {key: min(1, value) for key, value in
    #                                                       gene_contribution_dict_in_reaction.items()}
    #
    #                 genes_in_reaction_dict_for_promiscuity[
    #                     string_parsed_expr] = gene_contribution_dict_in_reaction_for_promiscuity
    ...
    #             per_reaction_gene_contribution_tuple_matrix[i][idx] = per_reaction_gene_contribution_in_sample[
    #                 reaction.id]
    #
    #         if promiscuity_method == "N":
    #             reaction_expression_matrix[:, idx] = [adjusted_expression_values_in_sample[reaction] for reaction in
    #                                                   cobra_model.reactions]
    #             for i, reaction in enumerate(cobra_model.reactions):
    #                 per_reaction_adjusted_gene_contribution_tuple_matrix[i][idx] = \
    #                     per_reaction_gene_contribution_in_sample[reaction.id]
    #         elif promiscuity_method == "Y":
    #             reaction_token_rule_dict = {
    #                 reaction.id: CombinationFileCreation.tokenize_gpr(reaction.gene_reaction_rule)
    #                 for reaction in cobra_model.reactions
    #             }
    #             gene_contribution_dict_in_sample = {key: max(1, value) for key, value in
    #                                                 gene_contribution_dict_in_sample.items()}
    ...
    #                     parsed_expr = CombinationFileCreation.parse_gpr(tokens)
    #                     string_parsed_expr = str(parsed_expr[0]) + str(parsed_expr[1])
    #                     promiscuity_gene_contribution_dict_in_reaction = genes_in_reaction_dict_for_promiscuity[
    #                         string_parsed_expr]
    #                     # in the case of trimmean, it should resolve to mean with only those that contribute #(all vals that are not 0 contribution)
    #                     # in the case of trimmin it will be a single value with min but that is also the only value (1 value is not 0 contribution)
    #                     # int he case of trimmax it with be a single value with max but that is also the only value (1 value is not 0 contribution)
    ...
    @staticmethod
    def assign_trimmable_gene_status(
            cobra_model: Model,
            expression_data: pd.DataFrame,
            should_filter_potential_trim_genes=True,
            method="M-value",  # or Tau or geNorm
            percentile=(2.5, 97.5),
            threshold=1,
            flat_reduction=2,
            capsys=None
        ...
        """
        Assigns a boolean to each gene in the model to indicate if it can be trimmed.
        The basis of this decision is that there might be problems with either the
        gene expression data or the GPR rules in model, which can lead to multi-enzyme
        reactions with AND rules getting a too low value as the minimum is taken.
        This will be more likely for big enzyme complexes that could potentially function
        with fewer subunits. Therefore a trimmedMin method was developed, however it is
        possible that a lowly expressed value in some samples but not others is actually
        biologically relevant. For this reason we assign a boolean to each gene to indicate
        whether it is stable enough to be potentially trimmed, or not when the gene varies
        among the samples provided. If it exceeds a conservative threshold then it cannot
        be trimmed.
        :param cobra_model:
            A cobra_compatible model
        :param expression_data:
            A pandas DataFrame with gene expression data
        :param should_filter_potential_trim_genes:
            A boolean to indicate if genes should be filtered based on their expression values
        :param method:
            The method to use for determining if a gene can be trimmed
        :param percentile:
            The percentile to use for the M-value method
        :param threshold:
            The threshold to use for the M-value method
        :param flat_reduction:
            The flat reduction to use for the M-value method
        :param capsys:
            The capsys fixture for capturing print statements
        :return:
            A dictionary with the gene ids as keys and a boolean as value
        """
        gene_ids = [gene.id for gene in cobra_model.genes if
                    gene.id in expression_data.index]
        if not should_filter_potential_trim_genes:
            return ({gene_id: True for gene_id in gene_ids},
                    {gene_id: {} for gene_id in gene_ids})
        amount_of_samples = expression_data.shape[1]
        genes_that_can_be_trimmed = {gene_id: False for gene_id in gene_ids}
        genes_values = {gene_id: {} for gene_id in gene_ids}

        if method == "M-value":
            percentile = list(percentile)
            expression_values = expression_data.loc[gene_ids].values
            percentiles = np.array([np.percentile(row, percentile,
                                                  # method = "closest_observation"
                                                  ) for row in expression_values])
            difference_between_percentiles = [
                max(percentiles[i][1] - flat_reduction, flat_reduction)
                / max(percentiles[i][0] - flat_reduction, flat_reduction)
                for i in range(len(percentiles))
            ]
            M_values = np.log2(
                difference_between_percentiles
            )
            min_values = np.min(expression_values, axis=1)
            max_values = np.max(expression_values, axis=1)
            for i, gene_id in enumerate(gene_ids):
                genes_values[gene_id] = {
                    "M_value": M_values[i],
                    "percentile_5": percentiles[i][0],
                    "percentile_95": percentiles[i][1],
                    "min_expression": min_values[i],
                    "max_expression": max_values[i],
                    "threshold": threshold,
                    "can_be_trimmed": M_values[i] <= threshold

                }
            return genes_that_can_be_trimmed, genes_values
        elif method == "Tau":
    ...
    def allocate_gene_values_to_reactions(
            ...
            should_filter_potential_trim_genes=True,
            GPR_OR_argument="sum",
            GPR_AND_argument="trimmin1",
            GPR_value=0.15,
            GPR_value_2=None,
            trim_method="M-value",
            trim_percentiles=(2.5, 97.5),
            trim_threshold=1,
            trim_flat_reduction=2,
            capsys=None
    ) -> pd.DataFrame:
        """
        Allocates gene values to reactions by applying GPR rules.
        It disambiguates any parts separated by "or" and "and" and
        applies the GPR_OR_argument and GPR_AND_argument. Ors will
        be solved before "ands" as "ors" fully separate the genes, while
        "ands" need to remain unsolved for trimming purposes.

        :param cobra_model:
            A cobra_compatible model
        :param expression_data:
            A pandas DataFrame with gene expression data
        :param promiscuity_method:
            The method to use for promiscuity, either "Y" or "N"
        :param should_filter_potential_trim_genes:
            A boolean to indicate if genes should be filtered based on their expression values
        :param GPR_OR_argument:
            The GPR rule used for "or" statements
        :param GPR_AND_argument:
            The GPR rule used for "and" statements
        :param GPR_value:
            If the first GPR rule uses a value, this value will be used
        :param GPR_value_2:
            If the second GPR rule uses a value, this value will be used
        :param trim_method:
            The method to use for determining if a gene can be trimmed
        :param trim_percentiles:
            The percentile to use for the M-value method
        :param trim_threshold:
            The threshold to use for the M-value method
        :param trim_flat_reduction:
            The flat reduction to use for the M-value method
        :param capsys:
            The capsys fixture for capturing print statements

        :return:
            A pandas DataFrame with the expression values per reaction
        """
        print("Starting")
        if promiscuity_method == "Y":
            raise NotImplementedError

        gene_ids = [gene.id for gene in cobra_model.genes]
        amount_of_samples = expression_data.shape[1]
        return_expression_per_reaction = pd.DataFrame(index=cobra_model.reactions,
                                                      columns=range(amount_of_samples))
        # expression_data = expression_data[expression_data.index.isin(gene_ids)]
        expression_data = expression_data[expression_data.index.isin(gene_ids)].copy()
        for gene_id in gene_ids:
            if gene_id not in expression_data.index:
                expression_data.loc[gene_id] = [-1] * amount_of_samples
        genes_in_reaction_dict = {reaction.id: [gene.id for gene in reaction.genes] for
                                  reaction in cobra_model.reactions}
        gpr_in_reaction_dict = {reaction.id: reaction.gene_reaction_rule for reaction in
                                cobra_model.reactions}

        # Find potentially trimmable genes, otherwise all can be trimmed
        genes_that_can_be_trimmed, _ = CombinationFileCreation.assign_trimmable_gene_status(
            cobra_model,
            expression_data,
            should_filter_potential_trim_genes,
            trim_method,
            trim_percentiles,
            trim_threshold,
            trim_flat_reduction,
        ...
        # Patterns to find and, or, and empty ()
        pattern_or = (r"("
                      r"(?:ENSG\d+(?:ENSG\d+)*|\d+(?:\.\d+)?)"
                      r"\s+or\s+"
                      r"(?:ENSG\d+(?:ENSG\d+)*|\d+(?:\.\d+)?)"
                      r"(?:\s+or\s+(?:ENSG\d+(?:ENSG\d+)*|\d+(?:\.\d+)?))*)")
        pattern_and = (r"("
                       r"(?:ENSG\d+(?:ENSG\d+)*|\d+(?:\.\d+)?)"
                       r"\s+and\s+"
                       r"(?:ENSG\d+(?:ENSG\d+)*|\d+(?:\.\d+)?)"
                       r"(?:\s+and\s+(?:ENSG\d+(?:ENSG\d+)*|\d+(?:\.\d+)?))*)")
        pattern_parentheses = (r"\((?:\s*\()*\s*(ENSG\d+(?:ENSG\d+)*"
                               r"|\d+(?:\.\d+)?)\s*(?:\)*\s*)\)")

        for sample_idx in range(amount_of_samples):
            expression_data_sample = expression_data.iloc[:, sample_idx]
            expression_per_reaction = {reaction_id: -1 for reaction_id in
                                       cobra_model.reactions}
            for reaction in cobra_model.reactions:
                current_gpr = gpr_in_reaction_dict[reaction.id]
                if current_gpr == "":
                    continue
                while_iterator = 0
                total_ors_and_ands = len(
                    re.findall("or", current_gpr) + re.findall("and", current_gpr))
                not_solved = True
                while not_solved:
                    simple_ors = re.findall(pattern_or, current_gpr)
                    for simple_or in simple_ors:
                        genes = simple_or.split(" or ")
                        expressions = [expression_data_sample[gene] for gene
                                       in genes if gene.startswith("ENSG")
                                       ] + [float(gene) for gene in genes if
                                            not gene.startswith("ENSG")]
                        if GPR_OR_argument == "sum":
                            combination_value = np.sum(expressions)
                            value_to_use = "".join([
                                str(gene)
                                for gene in genes if gene.startswith("ENSG")
                            ])
                            genes_that_can_be_trimmed[value_to_use] = all(
                                [genes_that_can_be_trimmed.get(gene, False) for
                                 gene in genes if gene.startswith("ENSG")]
                            )
                            expression_data_sample[value_to_use] = combination_value
                        elif GPR_OR_argument == "max":
                            index_gene = np.argmax(expressions)
                            value_to_use = genes[index_gene]
                        elif GPR_OR_argument.startswith("min"):
                            index_gene = np.argmin(expressions)
                            value_to_use = genes[index_gene]
                        current_gpr = current_gpr.replace(simple_or, str(value_to_use))

                    parentheses = re.findall(pattern_parentheses, current_gpr)
                    for parenthesis in parentheses:
                        current_gpr = current_gpr.replace(f"({parenthesis})",
                                                          parenthesis)

                    simple_ands = re.findall(pattern_and, current_gpr)
                    for simple_and in simple_ands:
                        genes = simple_and.split(" and ")
                        expressions = [expression_data_sample[gene] for gene
                                       in genes if gene.startswith("ENSG")
                                       ] + [float(gene) for gene in genes if
                                            not gene.startswith("ENSG")]
                        if GPR_AND_argument == "sum":
                            combination_value = np.sum(expressions)
                            value_to_use = "".join([
                                str(gene)
                                for gene in genes if gene.startswith("ENSG")
                            ...
                            expression_data_sample[value_to_use] = combination_value
                        elif GPR_AND_argument == "max":
                            index_gene = np.argmax(expressions)
                            value_to_use = genes[index_gene]
                        elif GPR_AND_argument.startswith("min"):
                            index_gene = np.argmin(expressions)
                            value_to_use = genes[index_gene]
                        elif GPR_AND_argument.startswith("trim"):
                            if should_filter_potential_trim_genes:
                                trimmable_genes = {gene: expression_data_sample[gene]
                                                   for gene in genes if gene.startswith(
                                        "ENSG") and genes_that_can_be_trimmed.get(gene,
                                                                                  False)}
                                no_trim_genes = {gene: expression_data_sample[gene] for
                                                 gene in genes if gene.startswith(
                                        "ENSG") and not genes_that_can_be_trimmed.get(
                                        gene, False)}
                            else:
                                trimmable_genes = {gene: expression_data_sample[gene]
                            ...
                            sorted_genes = dict(sorted(trimmable_genes.items(),
                                                       key=lambda item: item[1]))
                            # trim_count = int(len(sorted_genes) * GPR_value)
                            trim_count = int(len(sorted_genes) * 0.15)
                            # if len(sorted_genes) - 2 * trim_count >= 2:
                            #     trimmed_genes = dict(list(sorted_genes.items())[
                            #                          trim_count:len(
                            #                              sorted_genes) - trim_count])
                            trimmed_genes = {}
                            if len(sorted_genes) - 2 * trim_count >= 2:
                                trimmed_genes = dict(list(sorted_genes.items())[trim_count:-trim_count])

                            all_genes_and_values = no_trim_genes | trimmed_genes
                            all_genes = [gene for gene in all_genes_and_values.keys()]
                            all_values = [value for value in
                                          all_genes_and_values.values()]
                            if GPR_AND_argument == "trimmin1":
                                index_gene = np.argmin(all_values)
                                value_to_use = all_genes[index_gene]
                            elif GPR_AND_argument == "trimmin2":
                                index_gene = np.argmin(all_values)
                                value_to_use = all_genes[index_gene]
                            elif GPR_AND_argument == "trimmax":
                                index_gene = np.argmax(all_values)
                                value_to_use = all_genes[index_gene]
                            elif GPR_AND_argument == "trimsum":
                                combination_value = np.sum(all_values)
                                value_to_use = "".join([
                                    gene
                                    for gene in all_genes if gene.startswith("ENSG")
                                ])
                                genes_that_can_be_trimmed[value_to_use] = all(
                                    [genes_that_can_be_trimmed.get(gene, False) for
                                     gene in all_genes if gene.startswith("ENSG")]
                                )
                                expression_data_sample[value_to_use] = combination_value
                            elif GPR_AND_argument == "trimmean":
                                combination_value = np.mean(all_values)
                                value_to_use = "".join([
                                    gene
                                    for gene in all_genes if gene.startswith("ENSG")
                                ])
                                ...
                                expression_data_sample[value_to_use] = combination_value
                            elif GPR_AND_argument == "trimmedian":
                                combination_value = np.median(all_values)
                                value_to_use = "".join([
                                    gene
                                    for gene in all_genes if gene.startswith("ENSG")
                                ])
                                ...
                                raise NotImplementedError(
                                    f"Method {GPR_AND_argument} not implemented")
                        else:
                            raise NotImplementedError(
                                f"Method {GPR_AND_argument} not implemented")

                        current_gpr = current_gpr.replace(simple_and, str(value_to_use))
                    parentheses = re.findall(pattern_parentheses, current_gpr)
                    for parenthesis in parentheses:
                        current_gpr = current_gpr.replace(f"({parenthesis})",
                                                          parenthesis)
                    if while_iterator > total_ors_and_ands * 5:  # should be fine to have 2 but this (5) is to be safe
                        not_solved = False
                        warning(
                            f"Could not solve the GPR: reaction {reaction.id}\nin sample {sample_idx}\n original GPR: {gpr_in_reaction_dict[reaction.id]}\n current GPR: {current_gpr}")

                    while_iterator += 1
                    if len(re.findall("or", current_gpr)) + len(
                            re.findall("and", current_gpr)) == 0:
                        current_gpr = expression_data_sample[
                            current_gpr] if current_gpr.startswith("ENSG") else float(
                            current_gpr)
                        not_solved = False
                        original_gpr = gpr_in_reaction_dict[reaction.id]
                        for gene_id in genes_in_reaction_dict[reaction.id]:
                            original_gpr = original_gpr.replace(gene_id,
                                                                str(
                                                                    expression_data_sample[
                                                                        gene_id]))

                        result = float(current_gpr)
                        expression_per_reaction[reaction.id] = result
            expression_per_reaction = pd.Series(expression_per_reaction)
            return_expression_per_reaction[sample_idx] = expression_per_reaction
            print(f"Sample {sample_idx} done")
            if capsys is not None:
                with capsys.disabled():
                    print(f"Sample {sample_idx} done")
        return return_expression_per_reaction

    @staticmethod
    def apply_transformation_and_thresholding(
            expression_data: pd.DataFrame,
            linear_data: pd.DataFrame,
            transformation: str,
            thresholding_method: str,
            thresholding_value: float,
            local_global_no: str,
            second_local_global_no: str,
            percentile_value_mean: str,
            thresholding_value_2: float
        ...
        if local_global_no == "global" or "none":
            if transformation == "NT":
                ...
                raise NotImplementedError(f"The transformation method {transformation} is not implemented.")
        elif local_global_no == "local":
            ...
            raise ValueError(f"The thresholding method {local_global_no} is not recognized.")
        return gene_score
    ...
    def create_GPR_expression_data(
        ...
        CombinationFileCreation.save_partially_processed_data_files(
            combination_folder,
            global_options_dict,
            expression_data,
            "raw_data_",
        ...
        expression_data_non_metabolic_only = expression_data[expression_data.iloc[:, 0].isin(genes_not_in_model)]
        expression_data_metabolic_only = expression_data[~expression_data.iloc[:, 0].isin(genes_not_in_model)]
        ...
        gene_score = CombinationFileCreation.apply_transformation_and_thresholding(
            expression_data,
            linear_data,
            transformation,
            thresholding_method,
            thresholding_value,
            local_global_no,
            second_local_global_no,
            percentile_value_mean,
            thresholding_value_2
        )
        ...
        if GPR_value is not None and GPR_value in GPR_OR_argument:
            GPR_OR_argument = GPR_OR_argument.replace(GPR_value, "")
        elif GPR_value_2 is not None and GPR_value_2 in GPR_OR_argument:
            GPR_OR_argument = GPR_OR_argument.replace(GPR_value_2, "")
        GPR_AND_argument = GPR_method.lower().split(GPR_OR_argument)[0]
        if GPR_value is not None and GPR_value in GPR_AND_argument:
            GPR_AND_argument = GPR_AND_argument.replace(GPR_value, "")
        elif GPR_value_2 is not None and GPR_value_2 in GPR_AND_argument:
            GPR_AND_argument = GPR_AND_argument.replace(GPR_value_2, "")
        if GPR_value is not None:
            GPR_value = float(GPR_value)
        if GPR_value_2 is not None:
            GPR_value_2 = float(GPR_value_2)

        promiscuity_method = global_options_dict["__global_options"]["promiscuity_method"]
        should_filter_potential_trim_genes = global_options_dict["__global_options"].get(
            "should_filter_potential_trim_genes", True
        )
        trim_method = global_options_dict["__global_options"].get("trim_method", "M-value")
        trim_percentiles = global_options_dict["__global_options"].get("trim_percentiles", (2.5, 97.5))
        trim_threshold = global_options_dict["__global_options"].get("trim_threshold", 1)
        trim_flat_reduction = global_options_dict["__global_options"].get("trim_flat_reduction", 2)

        reaction_expression_matrix = CombinationFileCreation.allocate_gene_values_to_reactions(
            cobra_model,
            gene_score,
            promiscuity_method,
            should_filter_potential_trim_genes,
        ...
        CombinationFileCreation.save_partially_processed_data_files(
            combination_folder,
            global_options_dict,
            reaction_expression_matrix,
            "after_GPR_",
            "data",# TODO something wrong here, needs column, but also only has 2865 values?
            False,
            [reaction.id for reaction in cobra_model.reactions]
        )
        return (reaction_expression_matrix, gene_ids,
                expression_data_metabolic_only,
                expression_data_non_metabolic_only
                )

    @staticmethod
    def save_partially_processed_data_files(
            expression_data_location: str,
            global_options_dict: dict,
            data_to_save: Union[np.array, pd.DataFrame],
            file_name_prefix: str,
            file_name: str,
            ids_in_first_column: bool = True,
            ids: list = None,
    ):
        """
        Save the partially processed data in feather format

        :param expression_data_location: str
            The path to the expression data.
        :param global_options_dict: dict
            The dictionary with the global options for the preprocessing.
        :param data_to_save: np.array
            The data to save.
        :param file_name_prefix: str
            The prefix for the file name.
        :param file_name: str
            The file name to which the specific settings will be saved
        ...
        base_name = (
            f"{file_name_prefix}__"
            f"{file_name}__"
            f"{global_options_dict['__global_options']['transformation']}__"
            f"{global_options_dict['__global_options']['thresholding']}__"
            f"{global_options_dict['__global_options']['GPR_method']}__"
            f"{global_options_dict['__global_options']['promiscuity_method']}__"
            f"rev__.feather"
        )
        if not ids_in_first_column and ids is None:
            warnings.warn("The genes_ids are not provided, the data will be saved without the gene IDs.")

        if ids_in_first_column:
            sample_ids = [f"sample_{i + 1}" for i in range(data_to_save.shape[1] - 1)]
            index = data_to_save.iloc[:, 0]
            data_to_save = data_to_save.iloc[:, 1:]
            data_to_save.index = index
            data_to_save.columns = sample_ids
        else:
            sample_ids = [f"sample_{i + 1}" for i in range(data_to_save.shape[1])]
            data_to_save.index = ids
            data_to_save.columns = sample_ids

        file_location = os.path.join(expression_data_location, base_name)
        data_to_save.to_feather(file_location)
    ...
    def create_and_save_dataframes_from_matrices(
        ...
        reaction_expression_df = pd.DataFrame(reaction_expression_matrix,
                                              index=[reaction.id for reaction in cobra_model.reactions],
                                              columns=sample_ids)
        ...
        for sample in range(reaction_expression_matrix.shape[1]):
            if isinstance(reaction_expression_matrix, pd.DataFrame):
                reaction_expression_df_sample = pd.DataFrame(reaction_expression_matrix.iloc[:, sample],
                                                             index=[reaction.id for reaction in cobra_model.reactions],
                                                             columns=["reaction_expression"])
                
            elif isinstance(reaction_expression_matrix, np.ndarray):
                reaction_expression_df_sample = pd.DataFrame(reaction_expression_matrix[:, sample],
                                                         index=[reaction.id for reaction in cobra_model.reactions],
                                                         columns=["reaction_expression"])
            reaction_expression_df_sample.fillna(-1, inplace=True)
            if gene_contribution_matrix is not None:
                gene_contribution_df_sample = pd.DataFrame(gene_contribution_matrix.iloc[:, sample], index=genes_ids,
                                                           columns=["gene_contribution"])
                gene_contribution_df_location = os.path.join(samples_location,
                                                             gene_contribution_sample_file_name.format(sample + 1))
                gene_contribution_df_sample.to_csv(gene_contribution_df_location)
            reaction_expression_df_location = os.path.join(samples_location,
                                                           reaction_expression_sample_file_name.format(sample + 1))
    ...
    def create_and_save_per_reaction_matrices(
        ...
        for idx in range(len(per_reaction_gene_contribution_tuple_matrix)):
            for idx2 in range(len(per_reaction_gene_contribution_tuple_matrix[idx])):
                ...
                simple_per_reaction_adjusted_gene_contribution[idx][idx2] = {
                    ...
                    per_reaction_adjusted_gene_contribution_tuple_matrix[idx][idx2].items()
                    if values[1] > 0
                }
        ...
        per_reaction_gene_contribution_location = os.path.join(combination_folder,
                                                               per_reaction_gene_contribution_file_name)
        per_reaction_adjusted_gene_contribution_location = os.path.join(combination_folder,
                                                                        per_reaction_adjusted_gene_contribution_file_name)
        simple_per_reaction_gene_contribution_location = os.path.join(combination_folder,
                                                                      simple_per_reaction_gene_contribution_file_name)
        simple_per_reaction_adjusted_gene_contribution_location = os.path.join(combination_folder,
                                                                               simple_per_reaction_adjusted_gene_contribution_file_name)
        ...
        process_and_save_df(per_reaction_adjusted_gene_contribution_df,
                            per_reaction_adjusted_gene_contribution_location)
        process_and_save_df(simple_per_reaction_gene_contribution_df, simple_per_reaction_gene_contribution_location)
        process_and_save_df(simple_per_reaction_adjusted_gene_contribution_df,
                            simple_per_reaction_adjusted_gene_contribution_location)
    ...
    def transform_reversible_expression_to_irreversible(
        ...
        for idx, reaction in enumerate(rev_to_irrev_list):
            irreversible_expression_matrix[idx, :] = reaction_expression_matrix.iloc[idx, :]
            if len(rev_to_irrev_list[idx]) > 1:
                irreversible_expression_matrix[rev_to_irrev_list[idx][1] - 1, :] = reaction_expression_matrix.iloc[
                                                                                   rev_to_irrev_list[idx][0] - 1, :]
                ...
                if not all([x.startswith("MAM") for x in current_task_structure["input_metabolites"]]):
                    for idx3, met in enumerate(current_task_structure["input_metabolites"]):
                        if not met.startswith("MAM"):
                            ...
                            if not replaced:
                                raise ValueError(
                                    f"The input metabolite {met} of task {task_id} is not present in the model.")
                if not all([x.startswith("MAM") for x in current_task_structure["output_metabolites"]]):
                    for idx3, met in enumerate(current_task_structure["output_metabolites"]):
                        if not met.startswith("MAM"):
                            ...
                            if not replaced:
                                raise ValueError(
                                    f"The output metabolite {met} of task {task_id} is not present in the model.")
    ...
    def save_task_structure(
        ...
        try:
            ...
            if os.path.exists(task_structure_location):
                os.remove(task_structure_location)
            raise ValueError(f"The task structure could not be saved to {task_structure_location}.")

    @staticmethod
    def create_and_save_metadata_file(
            main_data_folder: str,
            global_options_dict: dict,
    )-> None:
        """
        Creates a .json file containing all the settings used to create the specific
        iteration of file in the combinatinos folder. This was added in version 1.05
        because with the addition of trimming thresholds and options, putting it all in
        main name of the folder became untannable.
        :param main_data_folder:
            The directory where the data is to be stored
        :param global_options_dict:
            Dictionary containing the options used to create and populate the folder
        :return:
            None
        """
        with open(os.path.join(main_data_folder, "metadata.json"), "w") as f:
            dump(global_options_dict, f)
    ...
    def populate_combination_files(
        ...
        if inputs_dict["rev_to_irrev_dict"] is None:
            cobra_model, rev_to_irrev_dict = CombinationFileCreation.create_closed_model(cobra_model,
                                                                                         global_options_dict)
            inputs_dict["model"] = cobra_model
            inputs_dict["rev_to_irrev_dict"] = rev_to_irrev_dict
            rev_to_irrev_name = f"{global_options_dict['__global_options']['model_version']}__rev_to_irrev_dict.json"
        ...
        if inputs_dict["expression_data"] is None:
            ...
            CombinationFileCreation.create_and_save_metadata_file(
                combination_folder,
                global_options_dict,
            )
    ...
    def master_create_combination_directory(
        ...
        # save metadata file including code version
        metadata_file = os.path.join(combination_folder, "metadata.json")
        CombinationFileCreation.save_with_tries(
            metadata_file,
            global_options_dict,
            dump,
            False,
...
class IndividualFileUpdating: # TODO
    @staticmethod
...
if __name__ == "__main__":
    methods_dict_for_improved = {
        "__global_options": {
            # Preprocessing
            "model_type": "irr",
            "tasklist": "metabolic_task_slp",
            "model_version": "from_github",
            "expression_dataset": "DCM_test",
            "transformation": "NT",
            "thresholding": "NT",
            "GPR_method": "trimmin1Sum",
            "should_filter_potential_trim_genes": False,
            "trim_filter_flat_reduction": 2,
            "trim_filter_percentiles": (2.5, 97.5),
            "trim_filter_threshold": 1,
            "promiscuity_method": "N",
            "check_if_input_files_correct": True,

            # Preprocessing and MILP runs
            ...
            "tasks": [1, 2, 3, 4, 5, 10, 20, 35],
            "samples": list(range(1, 322)),
            "unbounded_lower_bound_value": 1,
            "unbounded_upper_bound_value": 1000,
            "result_output_folder": r"C:\Ioana\_uni\honours\slp\algorithm\Results",
            "name_of_run": "Test_run_1",
            "solver_settings": {
                "TimeLimit": 2200,
                # "IntFeasTol": 1e-7,
                # "FeasibilityTol": 1e-7,
    ...
    # main_data_folder = r"C:\Users\Jelle\OneDrive\Documenten\GitHub\Metabolic_Task_Score\Data\Main_files\For_running"
